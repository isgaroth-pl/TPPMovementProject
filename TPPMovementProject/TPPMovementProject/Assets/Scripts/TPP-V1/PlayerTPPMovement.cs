    ...
    public class PlayerTPPMovement : MonoBehaviour
    {
        ...
        private void SetDesiredMoveDirectionVector(Vector3 inputVector)
        {
            if (inputVector.normalized.magnitude >= 0.1f)
            {
                ...
                //Debug.Log("" + Vector3.SignedAngle(transform.forward.normalized, flatMoveDirection.normalized, transform.up));
                //var checkAngle = Vector3.SignedAngle(transform.forward.normalized, flatMoveDirection.normalized, transform.up);
                //if (!(checkAngle > 100 || checkAngle < -100))
                desiredMoveDirection = Vector3.SmoothDamp(transform.forward.normalized, flatMoveDirection.normalized, ref _currentVelocityReference, 0.5f);
